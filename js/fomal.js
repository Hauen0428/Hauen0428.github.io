...
document.addEventListener('pjax:complete', function () {
  ...
  function createtime1() {
    var grt = new Date("02/06/2024 00:00:00"); //此处修改你的建站时间或者网站上线时间
  ...
  function createtime2() {
    ...
    setTimeout(
      console.warn.bind(
        console,
        "%c ⚡ Powered by Hauen %c 你正在访问Hauenの小家",
  ...
  function share_() {
    ...
    try {
      ...
      var subTitle = title.endsWith("| Hauen") ? title.substring(0, title.length - 14) : title;
      navigator.clipboard.writeText('Hauen的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
      new Vue({
        data: function () {
          this.$notify({
            title: "成功复制分享信息🎉",
  ...
  */
  ...
  function createtime() {
    ...
    var start = new Date("06/02/2024 00:00:00"); // 旅行者1号开始计算的时间
    var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
    var unit = (dis / 149600000).toFixed(6);  // 天文单位
    // 网站诞生时间
    var grt = new Date("06/02/2024 00:00:00");
    ...
    (currentTimeHtml =
      hnum < 18 && hnum >= 9
        ? `<img class='boardsign' src='https://picbed.hauen.cn//img/天亮了3.svg'height=30px;width=10px; title='白天是认真学习的好时机哦~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
        ...
        : `<img class='boardsign' src="https://picbed.hauen.cn//img/夜深了.svg" height=30px;width=10px; title='夜里可得认真回顾一天的知识哦~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
